...
class Conv:
    ...
    def __init__(self, num_kernels):
        ...
        self.kernels = np.random.randn(num_kernels, 3, 3) / 9
    ...
    def forward(self, input):
        ...
        output = np.zeros((height - 2, width - 2, self.num_kernels))
...
class MaxPool:
    ...
    def iterate_imgs(self, image):
        ...
        for i in range(new_height):
            for j in range(new_width):
                iter_img = image[(i * 2):(i * 2 + 2), (j * 2):(j * 2 + 2)]
                # 返回小图像矩阵及位置的生成器
                yield iter_img, i, j
    ...
    def forward(self, input):
        ...
        # 先初始化输出矩阵
        output = np.zeros((height // 2, width // 2, num_kernels))
...
class Softmax:
    ...
    def backprop(self, nabla_out, lr):
        ...
        for i, gradient in enumerate(nabla_out):
            ...
            out_t = -t_exp[i] * t_exp / (S ** 2)
            out_t[i] = t_exp[i] * (S - t_exp[i]) / (S ** 2)
...
def Train(image, label, lr=.005):
    ...
    return gradient3, loss, accuracy
