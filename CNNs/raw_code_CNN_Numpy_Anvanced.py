...
def img2col(x, ksize, stride):
    ...
    for i in range(feature_w):
        for j in range(feature_w):
            image_col[num] = x[i * stride:i * stride + ksize,
                               j * stride:j * stride + ksize, :].reshape(-1)
            num += 1
    return image_col
...
class Conv(object):
    ...
    def forward(self, x):
        ...
        for i in range(bx):
            ...
            feature[i] = (np.dot(image_col, kernel) +
                          self.b).reshape(feature_w, feature_w, nk) # 24，24，6
    ...
    def backward(self, delta, learning_rate):
        ...
        for i in range(bx):
            self.k_gradient += np.dot(self.image_col[i].T,
